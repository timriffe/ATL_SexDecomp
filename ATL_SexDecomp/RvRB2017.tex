...
Of course the disability prevalence is rarely a function of time-to-death
...
\section{Conclusions}
In Section~\ref{sec:morb} we show how simplistic time-to-death and age-patterns of morbidity interact with lifetable survivorship to yield opposite conclusions on morbidity compression under improving mortality. An fixed increasing age pattern of morbidity predicts a greater burden of morbidity under improved survival, but a fixed time-to-death pattern of morbidity predicts no change the average time spent in poor health, and therefore morbidity compression. In Section~\ref{sec:schematic} we show
Comparisons of period healthy life expectancy calculated from marginal age patterns of morbidity prevalence cannot be directly partitioned into morbidity and mortality effects if the joint age by time-to-death morbidity prevalence pattern varies at all by time-to-death. and on the precision with which a time-to-death pattern summarizes variation in the joint pattern. In Section~\ref{sec:schematic} we demonstrate the issue of for an index of instrumental activities of daily life. We demonstrate in Section~\ref{sec:bounds} that the degree of bias in partitioning effects depends on the shape and level of time-to-death variation. 
