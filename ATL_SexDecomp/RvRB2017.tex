...
IADL-disability prevalence broken down by age and TTD. Figure~\ref{fig:IADLsurf} shows these data as a surface, whose more horizontally running prevalence contour lines indicate that time-to-death is a more important time measure to describe prevalence in IADL than age. Figures~\ref{fig:IADLttd} and~\ref{fig:IADLage}, show the same prevalence data plotted with respect to the respective TTD and age margins. Of these, \ref{fig:IADLttd} yields a more compact set of patterns, which one could imaginably summarize with a single line without much loss of precision.  Thus our convenience assumption of fixed morbidity prevalence on the
