...
########################################
# TR: this graph rocks. Note: the data file gyall.Rdata is a deprecated data object
# from this project. It is used to extract the schematic TTD patterns.
# The code used to produce it has either been commented out
# or has been demoted to a zzz prefixed file. That doesn't matter because
# these could easily be invented to make the same point. Presently, empirical
# patterns of this kind can be extracted from the data object called ResultsP.Rdata
# This code would need to be reworked to account for that, not worth the effort at present.
########################################
