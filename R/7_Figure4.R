...
# TR: same object as used earlier in scripts
load("C:\\users\\vanraalte\\Dropbox\\sex decomp Tim\\Data\\resultsP.Rdata")

str(boot.results.list)
bl <- boot.results.list # giving it a shorter name
distype <- rep(0,82) #disability types
for(i in 1:82){
	distype[i] <- distype[[i]]$var
}
...
for(i in 1:82){
	TTDlist_F[[i]] <- mutate(data.frame(prev=rowMeans(bl[[i]]$Female$Surf,na.rm=T)),sex='f',
			varname=bl[[i]]$Female[[3]],TTD=0:12)
	TTDlist_M[[i]] <- mutate(data.frame(prev=rowMeans(bl[[i]]$Male$Surf,na.rm=T)),sex="m",
			varname=bl[[i]]$Female[[3]],TTD=0:12)
}
...
cairo_pdf("C:\\users\\vanraalte\\Dropbox\\sex decomp Tim\\figures\\DisbyTTDnew.pdf",width=8,height=4)
par(mfrow=c(1,3))

TTD = 0:12

plot(TTD,filter(TTDdf,varname=="iadl5_1",sex=='f')$prev,cex.axis=1.5,cex.lab=1.5,
		t="l",lwd=2,
		ylim=c(0,0.7),ylab="Disability prevalance",xlab="Time to Death (Years)")
lines(TTD,filter(TTDdf,varname=="iadl5_2",sex=='f')$prev,lwd=2,col="red")
lines(TTD,filter(TTDdf,varname=="iadl5_3",sex=='f')$prev,lwd=2,col="blue")
grid()
...
plot(TTD,filter(TTDdf,varname=="adl5_1",sex=='f')$prev,cex.axis=1.5,cex.lab=1.5,
		t="l",lwd=2,
		ylim=c(0,0.7),ylab="Disability prevalance",xlab="Time to Death (Years)")
lines(TTD,filter(TTDdf,varname=="adl5_2",sex=='f')$prev,lwd=2,col="red")
lines(TTD,filter(TTDdf,varname=="adl5_3",sex=='f')$prev,lwd=2,col="blue")
grid()
...
plot(TTD,filter(TTDdf,varname=="nh_now",sex=='f')$prev,cex.axis=1.5,cex.lab=1.5,
		t="l",lwd=2,
		ylim=c(0,0.7),ylab="Disability prevalance",xlab="Time to Death (Years)")
lines(TTD,filter(TTDdf,varname=="srhpoor",sex=='f')$prev,lwd=2,col="red")
lines(TTD,filter(TTDdf,varname=="name_mo",sex=='f')$prev,lwd=2,col="blue")
grid()
